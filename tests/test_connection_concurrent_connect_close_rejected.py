"""Concurrent ``connect()`` / ``close()`` on one ``DqliteConnection`` are rejected
via the synchronous ``_in_use`` claim. Without it both calls passed
``_check_in_use()`` then proceeded, building duplicate transports."""
...
async def test_concurrent_connect_calls_reject_one_via_in_use_guard() -> None:
    """A concurrent second ``connect()`` raises ``InterfaceError`` from
    ``_check_in_use`` rather than racing through."""
    conn = DqliteConnection("localhost:9001", timeout=1.0)

    # Slow open_connection so the second call lands while the first is in flight.
    ...
    try:
        ...
        await started.wait()
        # first is suspended inside open_connection; attempt a second.
...
async def test_concurrent_close_and_connect_rejected_via_in_use_guard() -> None:
    """Concurrent ``close()`` and ``connect()`` must produce an ``InterfaceError``
    from the second-arriving caller via the synchronous ``_in_use`` claim."""
    conn = DqliteConnection("localhost:9001", timeout=1.0)

    # A fake _protocol so close() has work to do.
    ...
    await started.wait()
    # close() is suspended on wait_closed; a concurrent connect() is rejected.
    with pytest.raises(InterfaceError, match="another operation"):
        await conn.connect()
    proceed.set()
    await close_task
