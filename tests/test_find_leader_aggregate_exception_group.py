"""``ClusterClient._find_leader_impl`` chains per-node failures via
``BaseExceptionGroup`` when more than one node contributed an exception,
so callers branching on the cause class see a deterministic result."""
...
async def test_find_leader_aggregate_keeps_narrow_chain_on_single_failure() -> None:
    """Single-failure case keeps the narrow chain so callers that branch
    on ``e.__cause__`` type continue to work."""
