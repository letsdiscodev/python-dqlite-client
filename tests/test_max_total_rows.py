"""max_total_rows / max_continuation_frames: validation and enforcement while draining rows."""
...
class TestValidator:
    def test_none_allowed(self) -> None:
        assert validate_positive_int_or_none(None, "max_total_rows") is None

    def test_positive_int_allowed(self) -> None:
        assert validate_positive_int_or_none(1, "max_total_rows") == 1
        assert validate_positive_int_or_none(10_000_000, "max_total_rows") == 10_000_000

    def test_zero_rejected(self) -> None:
        with pytest.raises(ValueError, match="max_total_rows must be > 0"):
            validate_positive_int_or_none(0, "max_total_rows")

    def test_negative_rejected(self) -> None:
        with pytest.raises(ValueError, match="max_total_rows must be > 0"):
            validate_positive_int_or_none(-1, "max_total_rows")
    ...
    def test_string_rejected(self) -> None:
        with pytest.raises(TypeError, match="max_total_rows must be int or None"):
            validate_positive_int_or_none("100", "max_total_rows")  # type: ignore[arg-type]
