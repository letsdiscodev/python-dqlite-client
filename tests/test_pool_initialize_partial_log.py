"""Pin the DEBUG breadcrumb logging the survivor-close count on
ConnectionPool.initialize partial failure."""
...
async def test_partial_failure_logs_survivor_count(
    ...
    # 2 succeeding conn mocks + 1 failing call; mocks record close() so the
    # test can assert the close loop ran.
    good1 = MagicMock()
