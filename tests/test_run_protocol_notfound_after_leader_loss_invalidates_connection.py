...
classification settled in done/ISSUE-862 / ISSUE-954. The
discriminator is therefore substring-based (the wire-side constant
...
async def test_notfound_lookup_stmt_does_not_invalidate_connection() -> None:
    ...
    The PEP 249 classifier still maps to InternalError (settled in
    done/ISSUE-862 / ISSUE-954); orthogonal to the invalidation
    decision here."""
    conn = DqliteConnection("localhost:9001")
    conn._db_id = 1
    conn._protocol = object()  # type: ignore[assignment]
